...
#自定义AI角色
def on_text_change():
    new_system_text = st.session_state.system_text
    for l, system in enumerate(st.session_state.messages):
        if system["role"] == "system" and l == 0:
            system["content"] = new_system_text
            break
#设置页面的配置项。标题，图标，页面布局。
st.set_page_config(
    page_title="AI聊天伴侣",
...
if 'system_text' not in st.session_state:
    st.session_state.system_text = "你叫小甜甜，你是一位温柔可爱的姑娘，一次说一句话。请用微信聊天的方式与用户聊天"
    st.session_state.messages.append({"role": "system", "content": st.session_state.system_text})
#存在历史会话，需要展示历史信息
for i, msg in enumerate(st.session_state.messages):
    if msg and i != 0:
        if msg["role"] == "system":
            # streamlit中没有system角色，必须再显示的的时候更换过来
            msg["role"] = "assistant"
...
with st.sidebar:
    ...
    st.text_area(
        ...
        value=st.session_state.system_text
    )
    # 侧边栏有历史消息，展示出来
    json_files = glob.glob("resource/data/*.json")
    if json_files:
        st.text("会话信息")
